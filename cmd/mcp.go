  ...
  icpcli mcp`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cfg.RateLimit.Enabled {
			limiter := mcpserver.NewRateLimiter(cfg.RateLimit.QueryPerMin, cfg.RateLimit.BlacklistPerMin)
			srv.AddReceivingMiddleware(limiter.Middleware())
			fmt.Fprintf(os.Stderr, "Rate limiting: query=%d/min blacklist=%d/min\n",
				cfg.RateLimit.QueryPerMin, cfg.RateLimit.BlacklistPerMin)
		}
